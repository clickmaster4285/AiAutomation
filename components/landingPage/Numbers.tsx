...
export default function Numbers() {
  const stats = [
    [
      ...
      "%",
      "Automation Accuracy",
      "Mean performance across our workflows since deployment.",
    ],
    [
      "75",
      "%",
      "Time Saved",
      "Average time reclaimed per workflow per employee.",
    ],
    [
      "3",
      "×",
      "Lead Conversion",
      "Lift in pipeline conversion since automation rollout.",
    ],
    [
      "40",
      "%",
      "Cost Reduction",
      "Average operational cost savings vs. manual ops.",
    ],
  ];
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 md:gap-0 border-t border-border pt-6 md:pt-12 md:divide-x divide-border">
          {stats.map(([n, suf, label, desc], i) => (
            ...
            >
              ...
              <p className="text-xs text-muted-foreground mt-1 md:mt-2 max-w-[14rem] mx-auto md:mx-0">
                {desc}
              </p>
            </motion.div>
          ))}
