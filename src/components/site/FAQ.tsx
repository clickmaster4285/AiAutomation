        ...
        <div className="flex flex-col md:grid md:grid-cols-[1fr_2fr] gap-8 md:gap-12 pt-6 md:pt-12">
          ...
          <div className="relative text-center md:text-left">
            <h2 className="display text-4xl sm:text-5xl md:text-6xl lg:text-7xl">
              Common<br /><span className="display-italic text-brand">Questions</span>
            </h2>
            ...
            <a href="#" className="inline-block mt-4 md:mt-6 bg-ink text-white text-sm px-4 md:px-5 py-2.5 md:py-3 rounded-sm hover:bg-brand transition-colors">
              Book a Call →
            </a>
            <img 
              src={robotMascot} 
            ...
            {faqs.map((f, i) => (
              <div key={f[0]}>
                ...
                >
                  ...
                  <span className={`size-6 md:size-7 rounded-full flex items-center justify-center text-xs transition-colors shrink-0 ${open === i ? "bg-brand text-white" : "bg-secondary text-ink"}`}>
                    {open === i ? "−" : "+"}
                  </span>
                </button>
                <motion.div
