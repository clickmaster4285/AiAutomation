  ...
  return (
    <section id="services" className="bg-paper py-12 md:py-24">
      <div className="mx-auto max-w-[90vw] md:max-w-[84vw] px-4 md:px-6">
        {/* Header row with Marker and eyebrow */}
        ...
        <div className="relative">
          {/* Floating dots — positional decorators - hidden on mobile, visible on md+ */}
          ...
          <div className="block md:hidden bg-border border border-border rounded-xl overflow-hidden">
            {services.map((s, i) => (
              <motion.div
                ...
                className={`bg-paper p-5 sm:p-6 border-b border-border last:border-b-0 group hover:bg-ink hover:text-white transition-colors duration-500`}
              >
                {s.tag && (
                ...
                <h3 className="display text-2xl sm:text-3xl mb-2">{s.title}</h3>
                <p className="text-sm sm:text-base text-muted-foreground group-hover:text-white/70 max-w-full">
                  {s.desc}
                </p>
                <div className="mt-4 eyebrow text-muted-foreground group-hover:text-brand text-xs sm:text-sm">
                  Learn more →
                </div>
              </motion.div>
            ))}
          ...
          <div className="hidden md:grid grid-cols-1 md:grid-cols-3 gap-px bg-border border border-border relative">
            {services.map((s, i) => (
              <motion.div
                key={s.title}
                initial={{ opacity: 0, y: 20 }}
